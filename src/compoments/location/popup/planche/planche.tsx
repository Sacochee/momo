...
export default function Planche({off} : {off : () => void}) {
    ...
    const [state1, setState1] = useState<pre>(mat.planche?.length && mat.planche.length > 0 ? { a : mat.planche[0].time, b : mat.planche[0].quantite} : {a : undefined, b : undefined})
    const [state2, setState2] = useState<pre>(mat.planche?.length && mat.planche.length > 1 ? { a : mat.planche[1].time, b : mat.planche[1].quantite} : {a : undefined, b : undefined})
    const [state3, setState3] = useState<pre>(mat.planche?.length && mat.planche.length > 2 ? { a : mat.planche[2].time, b : mat.planche[2].quantite} : {a : undefined, b : undefined})
  ...
  return (
    <div className={style.wrapper} ref={main}>
        ...
        <div>
             <Layout rmcount={()=>{off(); setMat({...mat, planche : undefined})}}>
    ...
    return(
        <section className={style.content}>
            ...
            <div className={style.box}>
                ...
                <div>
                    <p>Sous-total {count*getPrice(select!) ? count*getPrice(select!)  : 0} €</p>
                </div>
            </div>
