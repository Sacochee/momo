...
export default function Body({ off }: { off: () => void }) {
  ...
  const [state1, setState1] = useState<pre>(
    mat.body?.length && mat.body.length > 0
      ? { a: mat.body[0].time, b: mat.body[0].quantite }
      : { a: undefined, b: undefined }
  );
  const [state2, setState2] = useState<pre>(
    mat.body?.length && mat.body.length > 1
      ? { a: mat.body[1].time, b: mat.body[1].quantite }
      : { a: undefined, b: undefined }
  );
  const [state3, setState3] = useState<pre>(
    mat.body?.length && mat.body.length > 2
      ? { a: mat.body[2].time, b: mat.body[2].quantite }
      : { a: undefined, b: undefined }
  );

  useEffect(() => {
    if (mat.body?.length == 3) {
      setSupp({...supp, kase: true, case: true });
  ...
  return (
    <section className={style.content}>
      <div className={style.box}>
        ...
        <div>
          <p>Prix unité {getPriceBody(select!)} €</p>
        </div>
      </div>
      <div className={style.box}>
        <label className={style.label}>
        ...
        <div>
          <p>
            Sous-total{" "}
            {count * getPriceBody(select!) ? count * getPriceBody(select!) : 0}{" "}
            €
          </p>
        </div>
      </div>
